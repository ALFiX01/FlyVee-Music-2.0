...
module.exports = {
  name: "search",
  description: "Показывает результат треков на основе поискового запроса",
  usage: "[Песня]",
  permissions: {
  ...
  run: async (client, message, args, { GuildDB }) => {
    ...
    if (!SearchString)
      return client.sendTime(
        message.channel,
        `**Применение - **\`${GuildDB.prefix}search [поисковый запрос]\``
      );
    ...
    if (Searched.loadType == "NO_MATCHES")
      ...
      let Pages = songs.map((songz) => {
        let MappedSongs = songz.map(
          (s) =>
            `\`${s.index + 1}.\` [${s.title}](${
              s.uri
            }) \nДлительность: \`${prettyMilliseconds(s.duration, {
              colonNotation: true,
            })}\``
        );
      ...
      let msg = await message.channel.send(
        "**Введите номер композиции, которую хотите воспроизвести! Истекает через `35 секунд`**"
      );
      ...
      let SongID = await message.channel
        ...
        .catch(() => {
          er = true;
          msg.edit(
            "**Вы слишком долго не отвечали. Запустите команду еще раз, если хотите что-нибудь воспроизвести!**"
          );
        });
      if (er) return;
      /**@type {Message} */
      ...
      if (!parseInt(SongIDmsg.content))
        return client.sendTime(
          message.channel,
          "Пожалуйста, отправьте правильный идентификационный номер песни"
        );
      let Song = Searched.tracks[parseInt(SongIDmsg.content) - 1];
      if (!Song)
        return client.sendTime(
          message.channel,
          "Композиция по данному ID не найдена"
        );
    ...
    options: [
      {
        ...
        type: 3,
        required: true,
        description: "Введите название трека или URL-адрес, который хотите найти",
      },
    ],
    /**
     *
      ...
      if (search.match(client.Lavasfy.spotifyPattern)) {
        ...
        switch (res.loadType) {
          ...
          case "PLAYLIST_LOADED":
            ...
            return client.sendTime(
              interaction,
              `**Плейлист добавлен в очередь**: \n**${res.playlist.name}** \nВ очереди: **${res.playlistInfo.length} трека**`
            );
          case "SEARCH_RESULT":
            let max = 10,
              collected,
            ...
            const resultss = new MessageEmbed()
              .setDescription(
                `${results}\n\n\t**Введите номер трека, которую хотите воспроизвести!**\n`
              )
              .setColor(client.botconfig.EmbedColor)
              .setAuthor(
            ...
            try {
              ...
              return awaitchannel.send(
                "<:N_:993656004636053524>** ・ Вы не предоставили выбор**"
              );
            }
            ...
            if (index < 0 || index > max - 1)
              return awaitchannel.send(
                `<:N_:993656004636053524> ・ Введенное вами число больше или меньше общей суммы поиска. Применение - \`(1-${max})\``
              );
