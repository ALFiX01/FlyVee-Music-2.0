...
module.exports = {
  name: "seek",
  description: "Ищет позицию в треке",
  usage: "<Время с/м/ч>",
  permissions: {
  ...
  run: async (client, message, args, { GuildDB }) => {
    ...
    if (!player.queue.current.isSeekable)
      return client.sendTime(
        message.channel,
        "<:N_:993656004636053524>** ・ Я не могу найти этот трек!**"
      );
    let SeekTo = client.ParseHumanTime(args.join(" "));
    if (!SeekTo)
      return client.sendTime(
        message.channel,
        `**Применение - **\`${GuildDB.prefix}seek <number s/m/h>\` \n**Пример - **\`${GuildDB.prefix}seek 2m 10s\``
      );
    player.seek(SeekTo * 1000);
    message.react("<:Ready:985608370348257300>");
  },
  /*
    SlashCommand: {
        options: [
